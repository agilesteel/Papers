...
\begin{center}
Class \lstinline!Null! is the type of the null reference; it is a subclass of
every reference class (i.e., every class that itself inherits from
...
\pause
\begin{lstlisting}
scala> def whatsItGonnaBe(healthy: Boolean) =
     |    if(healthy) "dance" else ralph
whatsItGonnaBe: java.lang.String
\end{lstlisting}
\pause
\begin{lstlisting}
scala> println(whatsItGonnaBe(healthy = true))
dance

scala> println(whatsItGonnaBe(healthy = false))
java.lang.Exception
        at .<init>(<console>:7)
        ...
\end{lstlisting}
